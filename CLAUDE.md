...
- **No look-ahead in backtest:** at bar `t` an entry uses only bars `[..t]` and fills at `close[t]`; a position opened at `t` exits only on bars `> t`. Intraday timeframes: days are independent (fresh portfolio, EOD square-off) and run in parallel; trades merge in day order. The `1d` timeframe uses **positional mode** (`_simulate_range_daily`): one portfolio across the range, chronological, overnight holds, square-off at range end; `traded_today` = no re-entry per run and `DAILY_LOSS_LIMIT` = run-level loss stop.
