...
**The Kite-style manual-order form (`js/kiteForm.js`) is wired to the real paper-trading engine** (explicit user decision, 2026-09-01, reversing the form's original c.html-parity decorative-only behavior):
- Submit calls `POST /api/manual-order {direction}`; Exit calls `POST /api/manual-exit` (both in `app/api/dashboard.py`). These place/close the exact same `st.active_trade` the automated strategy uses — `app/services/bn_trade.py`'s `place_manual_order`/`force_close(..., label="MANUAL EXIT")` — a manual trading-desk override on the SAME single-trade slot, not a second/parallel trade concept. `place_manual_order` bypasses `evaluate_entry`'s strategy gates (it's a human decision, not an algo signal) but still goes through `open_trade_from_signal`, so risk parameters freeze from cfg exactly like an algo-fired trade, and lot size is always `cfg.BN_LOT_SIZE` (this engine has no position-sizing concept — the form's Qty field is fixed/display-only). Strike/expiry/premium are computed server-side via the real `bn_pricing.py` (ATM strike off the live BankNifty spot, realized-vol IV estimate) — there is no client-side pricing preview anymore. Order Type (Market/Limit) is cosmetic only: every fill in this engine is a synthetic Black-Scholes mark at submit time, there's no real order book for a limit price to rest on.
