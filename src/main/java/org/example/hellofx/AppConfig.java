...
public final class AppConfig {
    ...
    // Leader stocks used for signal alignment
    public static final List<String> LEADER_STOCKS = List.of(
        "HDFC BANK", "ICICI BANK", "AXIS BANK",
        "STATE BANK OF INDIA", "KOTAK MAHINDRA BANK", "INDUSIND BANK"
    );

    // Index weights (symbol → weight %)
    public static final Map<String, Double> INDEX_WEIGHTS;
    static {
        INDEX_WEIGHTS = new LinkedHashMap<>();
        INDEX_WEIGHTS.put("1333",  31.86); // HDFC BANK
        INDEX_WEIGHTS.put("4963",  20.14); // ICICI BANK
        INDEX_WEIGHTS.put("3045",  17.83); // SBI
        INDEX_WEIGHTS.put("1922",   8.79); // KOTAK
        INDEX_WEIGHTS.put("5900",   7.96); // AXIS
        INDEX_WEIGHTS.put("5258",   2.92); // INDUSIND
        INDEX_WEIGHTS.put("10666",  2.86); // PNB
        INDEX_WEIGHTS.put("10794",  2.40); // CANARA
        INDEX_WEIGHTS.put("11184",  1.40); // IDFC FIRST
        INDEX_WEIGHTS.put("21238",  1.35); // AU SMALL
        INDEX_WEIGHTS.put("1023",   1.19); // FEDERAL
    }

    // Per-stock qty thresholds (base / 1m)
